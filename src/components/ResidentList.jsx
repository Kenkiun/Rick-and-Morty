...
const ResidentList = ({location}) => {
  ...
  return (
   ...
   <section className='p-4 grid gap-6 auto-rows-auto grid-cols-[repeat(auto-fill,minmax(220px,1fr))] max-w-[1000px] mx-auto 
   '>
   ...
   <ul className='flex gap-4 justify-center py-4'>
      ...
      {
         arrayPages.map(page => <li onClick={() => setCurrentPage(page)} className={`p-3 text-white ${page === currentPage && 'bg-teal-950 text-white font-bold'} cursor-pointer`} key={page}>{page}</li>)
      }
